...
Scheduleで実行されるスキルは、**リモートのmainブランチにマージするところまで自動で行う**。
低リスク（情報蓄積・レポート生成）はmainに直接push。
高リスク（ハーネス改善）は別ブランチ→PR→マージ。

以下を提案（承認されたもののみ `/schedule` で設定）:

| スケジュール | 内容 | 推奨 | Git運用 |
|---|---|---|---|
| **source-sync** | Slack/Notion等から情報取得 | 毎日 9:00 | mainに直接push |
| **weekly-report** | 週次レポート生成 | 毎週金曜 16:00 | mainに直接push |
| **retro** | 振り返り（レポートのみ） | 毎週金曜 17:00 | mainに直接push |

各スケジュールの実行完了時:
1. 変更ファイルをgit add
2. commit（スキル名+日付のメッセージ）
3. push origin main（低リスク）または PR作成→マージ（高リスク）

次回のセッション開始時にgit pullで差分が自動表示される。

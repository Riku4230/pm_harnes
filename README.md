...
```bash
# 1. リポジトリをプロジェクトフォルダにコピー
cp -r pm_harnes ~/my-project
cd ~/my-project

# 2. Claude Codeを起動してセットアップ
claude
# → 「セットアップして」と言う
```

`/setup` が以下を順番に実行:
1. プロジェクトタイプ選択（personal / consulting / system_dev）
2. state/ docs/ の初期ファイル生成
3. git init + GitHub連携（scheduleに必要）
4. /schedule設定（weekly-report, retro）
5. 完了 → 「次はproject-initを実行してください」
...
`/project-init` がヒアリングしてdocs/state/を充実させる。

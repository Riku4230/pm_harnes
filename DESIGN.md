...
| 手段 | 実装 |
|---|---|
| JSONスキーマ | validate-state.sh: RISK mitigation必須、WBS循環検出、CHANGELOG append-only |
| 承認ゲート | approval-gate.sh: .claude/配下の直接編集をBlock |
...
│   │   ├── approval-gate.sh      PreToolUse: .claude/配下Block
│   │   └── validate-state.sh     PostToolUse: JSONスキーマ検証
│   ├── skills/                12スキル
│   │   ├── setup/                初回セットアップ
│   │   ├── source-sync/          外部ソース情報取得
│   │   ├── context-pack/         情報収集・整理
│   │   ├── meeting-import/       議事録取り込み
│   │   ├── wbs-update/           WBS管理
│   │   ├── risk-check/           リスク管理
│   │   ├── draft-update/         下書き生成（送信不可）
│   │   ├── context-sync/         ドキュメント同期
...
| PreToolUse(Edit\|Write) | approval-gate.sh | .claude/配下 → Block、state/docs/ → Log |
| PostToolUse(Edit\|Write) | validate-state.sh | JSONスキーマ + append-only + 循環検出 |
...
| context-pack | 各種ソースから情報収集・整理 | edit |
...
| context-sync | docs/ ↔ state/ 整合性確認 | readonly |
